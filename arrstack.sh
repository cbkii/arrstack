...
export SERVER_COUNTRIES DEFAULT_COUNTRY PROTON_CREDS_FILE PROTON_CREDS_FBAK GLUETUN_API_KEY
...
get_pf(){ curl -fsS "$GLUETUN_API/v1/openvpn/portforwarded"|jq -r '.port//empty'|grep -E '^[0-9]+$' | awk '$1>1024 && $1<65536{print$1}'; }
...
services:
  ...
  qbittorrent:
    ...
    healthcheck:
      test: ["CMD-SHELL", "curl -fsS http://localhost:8080/api/v2/app/version >/dev/null && [ -f /config/pf.port ]"]
      interval: 30s
      timeout: 10s
      retries: 5
      start_period: 60s
